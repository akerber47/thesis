...
into arcs alphai along the page boundaries, so each alphai cin Sa,ki. By
construction of tildeSa, lifting the alphai to tildeSa pieces and gluing cannot
yield closed curves.  It only yields a union of arcs, by (dag).

However, each alphai cin Sa,ki can also lift to a cover of the opposite page
boundary barSa,ki. Let Sb be the boundary component containing barSa,ki. Note
that it may be possible to have Sa = Sb. Write Sb,li = barSa,ki, we can see
that alphai in barPb,li, the set of "opposite parabolic arcs" that we defined
earlier.  So alphai lifts to S'b,li as a union of arcs, each of which connects
two different boundary components of S'b,li. This produces a set of arcs in
tildeSb.

We now claim that these arcs cannot be used to form closed curves in tildeSb.
Intuitively, this is because Sb does not remain parallel to alpha for long
enough, so we'll soon reach a piece of tildeSb where our lift can't continue.
To be precise, let tildealphai be a lift of alphai to tildeSb. Suppose alphai
has an endpoint at a spine Mc. If we look at how the boundary components of
a book of I-bundles behave near a spine, we that locally the two boundary sides
of a page are attached to sides of two different pages. In the terminology we
used earlier for gluing pages at a spine, gammai- and gammai+ are attached to
the attaching annulus boundary curves of two different pages:  gammai- attaches
to gamma(i+1)+, but gammai+ attaches to gamma(i-1)-. Since there are at least
three pages at Mc by (0), the (i-1)st and (i+1)st pages are distinct.
...
page, so there is no possible way to continue tildealphai. The same argument
applies at the opposite endpoint of tildealphai, which might lie above the same
spine Mc or a different one. This argument shows that lifts of arcs alphai cin
Sa,ki as "opposite parabolic arcs" to Sb have nothing on either side that they
can glue to in tildeSb. Since obviously it takes at least two attached arcs to
make a closed curve, there are no lifted closed curves in tildeSb.

Finally, we need to look at what happens to these alphai lifts when we do the
last gluing step. Since the alphai only form arcs in each piece of tildeS, the
...
traverse at least two pieces. This means that the alphai cannot lift and glue
...
when we glue tildeSa to itself in the final step. In order to attach the alphai
to form a lift of alpha, looking near the endpoints of an arc in tildeSa, we
have to glue to a lift of a continuation of alpha downstairs. But by the same
argument as above (where Sa and Sb are not parallel), Sa cannot remain locally
parallel to itself "on the other side". Even if the continuation of alpha lifts
...
part of it" for our parabolic alpha. Then by the same argument as for Sb alpha
