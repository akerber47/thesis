...
Let M be a compact n-manifold such that pi1M is LERF. Let M' to M be
a (possibly infinite-sheeted) cover such that pi1M' is finitely generated.
Suppose C cin M' is a compact subset which avoids the boundary of M. Then there
exists a intermediate cover M' to M'' to M, which is finite-sheeted over M,
such that the covering map M' to M'' is an embedding on C.

endthm

beginproof

See Long-Reid. Note that their proof is only stated for closed manifolds M, but
it carries over to all compact manifolds also if we restrict C to avoid the
boundary of M.
...
Let M be a compact 3-manifold such that pi1M is LERF. Let C be a compact
connected manifold or complex, and phi colon C immerse M a pi1-injective
immersion. Then there exists a finite sheeted cover M' to M such that phi lifts
to a map phi' colon C into M' which is homotopic to an embedding.

endthm

beginproof

Since C is a compact manifold or complex, pi1C is finitely generated. Let
H=phi*(pi1C), and let pH colon MH to M be the corresponding cover of M. By our
choice of H, phi lifts to a map phiH colon C to MH. We claim that phiH is
homotopic to an embedding. Since C is compact and phi is an immersion, it
suffices to show we can nicely homotope phiH to be injective.

Since pH*(pi1MH) = H = phi*(pi1C), phiH must induce an isomorphism on pi1 in
order to have pH circ phiH = phi. Let x1,x2 in C such that
phiH(x1)=phiH(x2)=y.  Let alpha cin C be a path from x1 to x2. Since phiH is
a pi1-isomorphism, phiH(alpha) must be nullhomotopic, 

Note that when we use this, both C and M will be aspherical, so an alternative
proof for that case is to invoke Whitehead's theorem to show phiH is a homotopy
equivalence, followed by some obstruction theory facts (?? - similar to the
ones invoked in Long-Reid) to show it can be homotoped to an embedding.
...
{\bf TODO now that I look at this thing, it's kind of a mess... should probably
group out / separate the topological vs proof convenient properties. }
%
% Talk to Ian about incompressible boundary.
...
% ^ maybe call this "topologically reduced" and move its definition earlier?
...
$M_c$.  By the facts (0), (1), and (2) we ensured above, we know that all these
...
$M_c$ by (0), the $(i-1)$st and $(i+1)$st pages are distinct.
...
% TODO
{\bf TODO fix this theorem / proof - wording is really vague, and I feel like
...
that intersect those pages. Then $(M,P)$ contains a surface satisfying
\eqref{E:qf} if and only if $(M',P')$ is nonempty and contains a surface
...
{\bf The following sections ended up being unnecessary. I'll keep it here just
...
% TODO rewrite
{\bf TODO Rewrite next steps after seeing how far we get.}
