...
can also combine these into a single cover $pi : sqcup \widetilde{S_{a,k}} -> sqcup Sa,k$
...
(dag) Each $tildeP(i) \cin \widetilde{S^{(i)}}$ does not contain any closed curves. That
...
Consider an arbitrary boundary component $tildegamma \cin d\widetilde{S^{(N)}} - sC$.  It
...
Let $tildegamma' \cin d\widetilde{S^{(N)}} - sC$ be an arbitrary leftover lift of
...
$tildegamma = tildegamma'$ gluing circle, otherwise $\widetilde{S^{(N)}}$ would already
...
\widetilde{S_b}. Note that it may be possible to have $Sa = Sb$, depending on how our
...
for the $Sb$ boundary component.  Locally, $tildealphai \cin \widetilde{S_b},li$, where
...
\cin \widetilde{S}$.  Cut $tildealpha$ along only the gluings between different
$\widetilde{S_b}$ done in the final gluing step. Each piece $tildealphaj$ now consists
...
% TODO fix this theorem / proof - wording is really vague, and I feel like
