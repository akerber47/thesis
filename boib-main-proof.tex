...
alpha in Qb (b neq a) also form a union of arcs. In fact, we claim that these
arcs can at most traverse a single page copy in the decomposition of Qb before
they stop.

Intuitively, this is because the only way to get a pattern component of alpha
in a page in Qb is to have a page B such that B cap Fa and B cap Fb are both
nonempty. That is, Fa and Fb contain the two sides of B. But then, at every
spine adjacent to B, Fa and Fb will diverge, so the alpha pattern cannot
continue in Qb (since it must extend to a cover of Fb).
