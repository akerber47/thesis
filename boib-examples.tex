...
\mytallfig{fig-bookex}{F:bookex}{Example of a book of $I$-bundles}
...
Instead of giving a full proof, we'll solve this example explicitly in
...
$\widetilde{F}$ if any (or, equivalently, all) lifts of that arc have both
endpoints in the same boundary component of $\widetilde{F}$.  Otherwise, we
say it's \emph{trans} for that cover. See Figure~\ref{F:3arcs}.

\mytallfig{fig-3arcs}{F:3arcs}{Lifting a standard set of 3 arcs in $\Si_{1,1}$}
...
Furthermore, we can choose any 2 of the 3 we wish to be trans with an
appropriate cover.
...
\myrotfig{fig-boibexample}{F:ex1}{Construction of $S$}
...
derived a method for gluing that avoided creating closed loops, by taking
