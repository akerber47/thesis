...
decomposes into the \emph{page boundary} $\pi^{-1}(\bd Si)$ and the
\emph{gluing annuli} $cup \bd \pi^{-1}(x)$. Note that the gluing annuli is in
fact a union of annuli. The reason for these terms is the following definition.
...
gluing annuli to disjoint embedded annuli in the boundaries of the solid tori.
If we let sA be the annuli we glue along, our resulting manifold can be written
as $sB cup_sA sC$.
...
and the components of sA as \emph{bindings}.

Algebraically, a book of I-bundles corresponds to a particularly simple graph
of groups. Half of the groups (the spines) are just copies of $Z$.
...
Let's look at the boundary components of a book of $I$-bundles.
% TODO explain "wrapping around" etc
