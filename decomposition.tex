...
properly embedded in Mi-2, so A cap D is a union of disjoint properly embedded
arcs and curves in D.
...
disk in Mi-1. But bd Mi is incompressible in Mi-1, so bd D must bound a disk in
bd Mi-1.

Furthermore, we claim that for odd i, Mi is not boundary parallel in Mi-2.
